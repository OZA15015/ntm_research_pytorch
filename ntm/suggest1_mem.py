...
class NTMMemory(nn.Module):
    ...
    def _similarity(self, k, β): 
        ...
        cos = torch.ceil(β * (((repre_memory+1e-16)*(k+1e-16)).sum(dim=-1)/torch.max((repre_memory+1e-16).norm(dim=-1)*(k+1e-16).norm(dim=-1), torch.tensor([[1e-8]]))).clamp(min=0)) + 1e-16
        ...
        for i in range(cos.size()[1]):
            # w[0, index_mul(std_id_list, i)] = cos[0, i] 元々これだけ
            """
            if(cos[0, i] > 1e-16):
