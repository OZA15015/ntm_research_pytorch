...
class my_algo_fn(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        w = torch.zeros(grad_input.size()[-1]*8, grad_input.size()[-1]*8)
        for i in range(grad_input.size()[1]):
            w[0, index_mul(std_id_list, i)] = J[0, i]
...
class NTMMemory(nn.Module):
    ...
    def cos_sim(self, v1, v2):
        ...
        cl_list.append(math.ceil(tmp))
        return tmp #x以上の整数を返す
